...
#endif /* HAVE_SNMP */
/* A counter for counting number of prefixes we are checking after link up  or node root cause event*/
int global_prefix_counter_initialization = 0;
...
struct event_affected_prefix_list* event_affected_prefix_list_head = NULL;
...
{
    zlog_debug("*********** for  %s we are going to get the number of affected prefixes",event_id);
    struct time_stamp_ds * temp = (*head_ref);
    ...
    while(temp != NULL)
    {
        ...
        struct event_affected_prefix_list * my_temp = temp -> event_affected_prefix_list_value ;
        while(my_temp != NULL)
        {
            zlog_debug("*********** we counted! ");
...
struct event_affected_prefix_list * get_prefix_list_affected_by_event(struct time_stamp_ds** head_ref,char * event_id)
{
    ...
    while(temp != NULL)
    {
        ...
        return temp -> event_affected_prefix_list_value;
    }
        temp = temp -> next;
    }
    return NULL;
...
struct time_stamp_ds *  get_list_of_unfizzled_updates(struct time_stamp_ds** head_ref,char * prefix,char * aspath_str_value,struct peer * received_from_peer)
{
    ...
    while(temp != NULL)
    {
        ...
        while(my_temp != NULL)
        {
            ...
            {
                zlog_debug("*********** and going to add new time stamp********** from line 314 for prefix %s and %s",my_temp -> prefix,prefix);
                add_new_time_stamp(&result,temp->event_id,temp->time_stamp,temp->received_from_peer->as,NULL,NULL,temp->received_from_peer,NULL,"aspath");
            }
            my_temp = my_temp -> next;
        }
...
struct time_stamp_ds * get_list_of_unfizzled_time_stamps(struct time_stamp_ds** head_ref,char * prefix,char * in_time_stamp,char * in_event_id,struct peer * received_from_peer)
{
    ...
    while(temp != NULL)
    {
        ...
        while(my_temp != NULL)
        {
            ...
            if(strcmp (my_temp -> prefix,prefix)==0)
            {
                zlog_debug("*********** and  going to add new time stamp********** from line 352 for prefix %s and %s as this is for withdraw",my_temp -> prefix,prefix);
                add_new_time_stamp(&result,temp->event_id,temp->time_stamp,temp->received_from_peer->as,NULL,NULL,temp->received_from_peer,NULL,"aspath");
            }
            my_temp = my_temp -> next;
        }
...
void add_affected_prefix_to_prefix_list(struct event_affected_prefix_list** head_ref,struct  prefix * prefix_struct)
{
    struct event_affected_prefix_list * new_node = (struct event_affected_prefix_list*) malloc(sizeof(struct event_affected_prefix_list));
...
void add_new_time_stamp(struct time_stamp_ds** head_ref,char * in_event_id,char * in_time_stamp_id,long AS_owner_id,struct update_prefix_list* pl,struct event_affected_prefix_list* eapl,struct peer * received_from_peer,struct attr * attr,char * aspath_value)
{
    ...
    // zlog_debug("*********** going to add %s as aspathvalue   ********** ",attr->aspath->str);
    ...
    new_node->processed_value= false;
    // if (attr!=NULL)
    // memcpy(new_node->attr_value, attr, sizeof(new_node->attr_value) );
    // else
    //     new_node->attr_value = NULL;


    new_node->attr_value = attr;
    new_node->upl = pl;
    new_node->saved_prefixes = pl;
    new_node->event_affected_prefix_list_value = eapl;
    new_node->next = (*head_ref);
    (*head_ref) = new_node;

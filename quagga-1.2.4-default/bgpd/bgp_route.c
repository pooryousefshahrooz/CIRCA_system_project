...
bgp_info_add (struct bgp_node *rn, struct bgp_info *ri)
{
  ...
  zlog_debug ("*****************************in bgp_info_add for %s ***********************!!!",ri->peer->host);
  struct bgp_info *top;
  ...
  switch (bgp_node_table (rn)->type)
    {
      ...
      /* Announcement to peer->conf.  If the route is filtered,
        ...
        {
          if (selected)
          zlog_debug ("*******1******call bgp_adj_out_set from bgp_process_announce_selected *********** peer is %s and selected is %s \n",peer->host,selected->peer->host);
          else
            zlog_debug ("*************call bgp_adj_out_set from bgp_process_announce_selected *********** peer is %s and selected is null!!! \n",peer->host);

          bgp_adj_out_set (rn, peer, p, &attr, afi, safi, selected);
        }
        else
        {
            zlog_debug ("*************  calling bgp_adj_out_unset from bgp_process_announce_selected ***********\n");
      ...
      case BGP_TABLE_RSCLIENT:
        ...
        {
          zlog_debug ("*******2******call bgp_adj_out_set from bgp_process_announce_selected *********** peer is %s and selected is %s \n",peer->host,selected->peer->host);
          bgp_adj_out_set (rn, peer, p, &attr, afi, safi, selected);
        }
        else
        {
        zlog_debug ("*************  2. calling bgp_adj_out_unset from bgp_process_announce_selected ***********\n");
  ...
  if (CHECK_FLAG (rsclient->sflags, PEER_STATUS_GROUP))
    {
      if (rsclient->group)
		...
		UNSET_FLAG (new_select->flags, BGP_INFO_MULTIPATH_CHG);
            ...
            zlog_debug ("*************call bgp_process_announce_selected from bgp_process_main for new_select ***********\n");
      ...
      if (new_select)
	{
      ...
      zlog_debug ("2. *************call bgp_process_announce_selected from bgp_process_main for new_select ***********\n");
...
bgp_process_main (struct work_queue *wq, void *data)
{
  ...
  /* Best path selection. */
  zlog_debug ("1 ************************** We are adding Best path selection for old_and_new !!***********\n");
  zlog_debug ("1 ************************** We are adding Best path selection for old_and_new !!***********\n");
  zlog_debug ("1 ************************** We are adding Best path selection for old_and_new !!***********\n");
  zlog_debug ("1 ************************** We are adding Best path selection for old_and_new !!***********\n");
      ...
      if (! CHECK_FLAG (old_select->flags, BGP_INFO_ATTR_CHANGED))
        {
          ...
          {
            zlog_debug ("0.1 We are adding FIB entries using bgp_zebra_announce for %s ***********\n",new_select->peer->host);
...
bgp_process (struct bgp *bgp, struct bgp_node *rn, afi_t afi, safi_t safi)
{
  ...
  switch (bgp_node_table (rn)->type)
    {
      case BGP_TABLE_MAIN:
        zlog_debug ("*****************************1.BGP_TABLE_MAIN work_queue_add***********************!!!");
        work_queue_add (bm->process_main_queue, pqnode);
        break;
      case BGP_TABLE_RSCLIENT:
        zlog_debug ("*****************************1.BGP_TABLE_RSCLIENT work_queue_add***********************!!!");
      ...
      int sub_type, struct prefix_rd *prd, u_char *tag)
{
  ...
  zlog_debug ("*****************************calling  bgp_info_add from bgp_update_rsclient ***********************!!!");
	    ...
	    struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
  ...
  if (! soft_reconfig && CHECK_FLAG (peer->af_flags[afi][safi], PEER_FLAG_SOFT_RECONFIG)
      && peer != bgp->peer_self)
  {
    zlog_debug ("***************************** When peer's soft reconfiguration enabled.  Record input packet in Adj-RIBs-In %s *****************************",peer->host);
  ...
  if (bgp_input_filter (peer, p, attr, afi, safi) == FILTER_DENY)
    {
      reason = "filter;";
      zlog_debug ("***************************** go to filtered!!! %s *****************************",peer->host);
  ...
  zlog_debug ("*****************************  %s ***************************** \n ",peer->host);
  zlog_debug ("***************************** bgp_attr_intern  %s ***************************** \n ",peer->host);

  attr_new = bgp_attr_intern (&new_attr);
  zlog_debug ("***************************** did bgp_attr_intern  %s ***************************** \n",peer->host);
  ...
  if (ri)
          ...
          && attrhash_cmp (ri->attr, attr_new))
	{
	  if (CHECK_FLAG (bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING)
	    ...
	    {
	      if (BGP_DEBUG (update, UPDATE_IN))  
		  zlog (peer->log, LOG_DEBUG, "%s 55 rcvd %s/%d",
		  peer->host,
		  inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
		  p->prefixlen);
      ...
      /* Received Logging. */
      if (BGP_DEBUG (update, UPDATE_IN))  
	zlog (peer->log, LOG_DEBUG, "%s 33 rcvd %s/%d",
	      peer->host,
	      inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
	      p->prefixlen);
  ...
  if (BGP_DEBUG (update, UPDATE_IN))  
    {
      zlog (peer->log, LOG_DEBUG, "%s 44 rcvd %s/%d",
	    peer->host,
	    inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
	    p->prefixlen);
    }
  ...
  /* Register new BGP information. */
  zlog_debug ("*****************************Register new BGP information to %s *****************************",peer->host);
  zlog_debug ("*****************************calling  bgp_info_add from bgp_update_main ***********************!!!");
  ...
  /* Process change. */
  zlog_debug ("***************************** Process change. to %s *****************************",peer->host);
            ...
            struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
  ...
  struct bgp *bgp;
  int ret;
  zlog_debug ("*************    calling bgp_update_main    ***********\n");
  ...
  for (ALL_LIST_ELEMENTS (bgp->rsclient, node, nnode, rsclient))
    {
      ...
      {
        zlog_debug ("*************    calling bgp_update_rsclient    ***********\n");
                   ...
                   struct bgp_table *table, int rsclient)
{
  ...
  for (rn = bgp_table_top (table); rn; rn = bgp_route_next(rn))
      ...
      if (CHECK_FLAG (ri->flags, BGP_INFO_SELECTED) && ri->peer != peer)
	{
         ...
         {
        zlog_debug ("************* 2. call bgp_adj_out_set from bgp_announce_table *********** peeer is %s , selected is ri  %s \n",peer->host,ri->peer->host);
	     bgp_adj_out_set (rn, peer, &rn->p, &attr, afi, safi, ri);
    }
	  else
    {
       zlog_debug ("*************  calling bgp_adj_out_unset from bgp_announce_table ***********\n");
                            ...
                            afi_t afi, safi_t safi)
{
  ...
  /* Register new BGP information. */
  zlog_debug ("*****************************calling  bgp_info_add from bgp_static_update_rsclient ***********************!!!");
			...
			struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
  ...
  /* Register new BGP information. */
  zlog_debug ("*****************************calling  bgp_info_add from bgp_static_update_main ***********************!!!");
                        ...
                        struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
  ...
  /* Register new BGP information. */
  zlog_debug ("***************************** calling  bgp_info_add from bgp_static_update_safi ***********************!!!");
  bgp_info_add (rn, new);
      ...
      new = info_make(ZEBRA_ROUTE_BGP, BGP_ROUTE_AGGREGATE, bgp->peer_self,
		      bgp_attr_aggregate_intern(bgp, origin, aspath, community,
						aggregate->as_set,
                                                atomic_aggregate), rn);
      SET_FLAG (new->flags, BGP_INFO_VALID);
      zlog_debug ("*****************************calling  bgp_info_add from bgp_aggregate_route ***********************!!!");
      bgp_info_add (rn, new);
      bgp_unlock_node (rn);
      bgp_process (bgp, rn, afi, safi);
    }
      ...
      new = info_make(ZEBRA_ROUTE_BGP, BGP_ROUTE_AGGREGATE, bgp->peer_self,
		      bgp_attr_aggregate_intern(bgp, origin, aspath, community,
						aggregate->as_set,
                                                atomic_aggregate), rn);
      SET_FLAG (new->flags, BGP_INFO_VALID);
      zlog_debug ("*****************************calling  bgp_info_add from bgp_aggregate_add ***********************!!!");
      bgp_info_add (rn, new);
      bgp_unlock_node (rn);
		      ...
		      u_int32_t metric, u_char type, route_tag_t tag)
{
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
    ...
    zlog_debug ("*****************************calling  bgp_info_add from bgp_redistribute_add ***********************!!!");
